        ...
        public async Task SendOtpEmailAsync(string email, string otp)
        {
            ...
            try
            {
                _logger.LogInformation(
                    "STEP 1: Sending API request to Brevo for {Email}...",
                    email
                );
                ...
                {
                    _logger.LogInformation(
                        "SUCCESS: OTP delivered via Brevo API to {Email}",
                        email
                    );
                }
                else
                {
                    ...
                    _logger.LogError(
                        "FAILED: Brevo API returned {Status}. Error: {Error}",
                        response.StatusCode,
                        errorDetails
                    );
            ...
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    "CRITICAL: Exception occurred while calling Brevo API for {Email}",
                    email
                );
                throw;
            }
