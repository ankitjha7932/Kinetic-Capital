    ...
    return (
      <div className="flex flex-col items-start gap-1">
        ...
        {isPositive || isNegative ? (
          <div className={`flex items-center gap-0.5 px-1.5 py-0.5 rounded-md text-[10px] font-black uppercase transition-colors ${
          ...
          <div className="flex items-center gap-0.5 px-1.5 py-0.5 rounded-md text-[10px] font-black text-slate-300 bg-slate-50 uppercase">
        ...
        <table className="w-full text-left border-collapse table-fixed min-w-[1000px]">
          <thead>
            <tr className="bg-slate-900">
              <th className="w-[240px] p-6 text-xs font-black text-slate-400 uppercase tracking-[0.2em] sticky left-0 z-30 bg-slate-900 border-r border-slate-800 shadow-[4px_0_10px_rgba(0,0,0,0.3)]">
                Ownership Category
              </th>
              {quarters.map((q) => (
                <th key={q} className="p-6 text-xs font-black text-slate-400 uppercase tracking-[0.2em] whitespace-nowrap">
                  {q}
                </th>
              ))}
            </tr>
          ...
          <tbody className="divide-y divide-slate-100">
            {history.map((row, idx) => {
              ...
              return (
                ...
                >
                  <td className={`p-6 font-black text-sm sticky left-0 z-20 whitespace-nowrap border-r border-slate-100 shadow-[4px_0_8px_-4px_rgba(0,0,0,0.15)] transition-colors 
                    ${isSelected 
