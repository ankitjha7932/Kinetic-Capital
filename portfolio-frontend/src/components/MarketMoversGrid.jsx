...
function Sparkline({ data, positive, width = 72, height = 28 }) {
  ...
  // baseline = average of last 7 values
  ...
  return (
    ...
    >
      ...
      <div className="mmg-name">
        <div className="mmg-sym">{stock.symbol}</div>
      ...
      <div className="mmg-vol">
        {isVol ? (
          ...
          <>
            <div style={{ fontSize:11, color:"#64748b" }}>{(stock.volume/1e5).toFixed(1)}L</div>
  ...
  return (
    <>
      <style>{`
        ...
        .mmg-row:hover { background: #fafbff; }
        ...
        /* Hide sparkline + vol columns on very small screens */
        @media (max-width: 380px) {
          .mmg-spark { display: none; }
          .mmg-vol   { display: none; }
          .mmg-price { min-width: 80px; }
        }

        /* ─── SEE MORE ──────────────────────────── */
        .mmg-more-btn {
          margin-top: 6px; width: 100%; padding: 10px 0;
        ...
        .mmg-more-btn:hover { color: #3730a3; }
      `}</style>
      ...
      {list.length > PREVIEW_COUNT && (
        <button className="mmg-more-btn" onClick={() => setExpanded(!expanded)}>
          {expanded
            ? <><ChevronDown size={13}/> Show less</>
            : <><ChevronRight size={13}/> See all {list.length} stocks</>
          }
        </button>
      )}
    </>
