  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 xl:grid-cols-7 gap-6">
        ...
        <div className="xl:col-span-2 bg-white rounded-[2rem] p-6 shadow-sm border border-slate-100 flex flex-col items-center min-h-[600px]">
          ...
          <div className="flex-1 w-full relative min-h-[320px]">
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none z-10 translate-y-2">
              ...
              <span className="text-3xl font-black text-slate-900 tabular-nums tracking-tight text-center px-4">
              ...
              {pieData.map((entry) => (
                <div 
                  ...
                  className={`flex items-center gap-2.5 p-1.5 rounded-xl transition-all duration-300 cursor-default ${
                    displayData?.name === entry.name ? 'bg-slate-50 translate-x-1' : 'opacity-60 hover:opacity-100'
                  }`}
                >
        ...
        {/* --- RIGHT: DATA TERMINAL (TOUCH-SAFE & STICKY) --- */}
        <div className="xl:col-span-5 bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden flex flex-col">
          <div className="p-8 flex justify-between items-center border-b border-slate-50">
            ...
            <div className="hidden sm:block text-right">
              <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest bg-slate-50 px-2 py-1 rounded whitespace-nowrap">Values in ₹ Crores</span>
            </div>
          </div>
              ...
              <table className="w-full table-fixed border-separate border-spacing-0">
                <thead>
                  <tr className="bg-[#1e293b] text-white uppercase text-[9px] font-black tracking-widest whitespace-nowrap">
                    ...
                    <SortHeader label="Div Yld" sKey="divYield" config={sortConfig} setConfig={setSortConfig} />
                    <SortHeader label="NP Qtr" sKey="netProfitQtr" config={sortConfig} setConfig={setSortConfig} />
                    <SortHeader label="Profit Var" sKey="profitVarQtr" config={sortConfig} setConfig={setSortConfig} />
                    <SortHeader label="Sales Qtr" sKey="salesQtr" config={sortConfig} setConfig={setSortConfig} />
                    <SortHeader label="Sales Var" sKey="salesVarQtr" config={sortConfig} setConfig={setSortConfig} />
                    <SortHeader label="ROCE" sKey="roce" config={sortConfig} setConfig={setSortConfig} isLast={true} />
                  </tr>
                </thead>
                <tbody className="text-[13px] font-black text-slate-900">
                  {sortedPeers.map((peer) => {
                    const isProfitNeg = peer.profitVarQtr?.toString().startsWith('-');
                    const isSalesNeg = peer.salesVarQtr?.toString().startsWith('-');
                    
                    return (
                      <tr key={peer.name} className="group transition-colors">
                        <td className={`p-5 whitespace-nowrap sticky left-0 z-20 border-b border-slate-50 shadow-[2px_0_5px_rgba(0,0,0,0.05)] transition-colors
                          ${peer.isCurrent ? 'bg-[#f5f7ff]' : 'bg-white group-hover:bg-[#f8fafc]'}`}>
                          {/* TOUCH SAFE NAVIGATION: block, w-full, z-30 */}
                          <button 
                            onClick={(e) => {
                              e.stopPropagation();
                              if (peer.symbol) navigate(`/stock/${peer.symbol}`);
                            }} 
                            className="relative z-30 text-indigo-600 hover:underline text-left font-black block w-full cursor-pointer touch-manipulation"
                          >
                            {peer.name}
                          </button>
                        </td>
                        <td className="p-5 text-center font-black border-b border-slate-50 bg-white group-hover:bg-slate-50/80">{peer.pe}</td>
                        <td className="p-5 text-center font-black border-b border-slate-50 bg-white group-hover:bg-slate-50/80">{formatIndian(peer.marketCap, true)}</td>
                        <td className="p-5 text-center font-black text-slate-500 border-b border-slate-50 bg-white group-hover:bg-slate-50/80">{peer.divYield}</td>
                        <td className="p-5 text-center font-black border-b border-slate-50 bg-white group-hover:bg-slate-50/80">{formatIndian(peer.netProfitQtr)}</td>
                        <td className={`p-5 text-center font-black border-b border-slate-50 bg-white group-hover:bg-slate-50/80 ${isProfitNeg ? 'text-rose-600' : 'text-emerald-600'}`}>{peer.profitVarQtr}</td>
                        <td className="p-5 text-center font-black border-b border-slate-50 bg-white group-hover:bg-slate-50/80">{formatIndian(peer.salesQtr)}</td>
                        <td className={`p-5 text-center font-black border-b border-slate-50 bg-white group-hover:bg-slate-50/80 ${isSalesNeg ? 'text-rose-600' : 'text-emerald-600'}`}>{peer.salesVarQtr}</td>
                        <td className="p-5 text-center font-black border-b border-slate-50 bg-white group-hover:bg-slate-50/80 text-slate-700">{peer.roce}</td>
                      </tr>
                    );
                  })}
                </tbody>
